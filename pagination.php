...
$sql = "SELECT * FROM candidateregdata WHERE ( `qua` LIKE  '%$searchQua%' AND `totalExp` LIKE  '%$searchExp%' ) ORDER BY email ASC LIMIT $start_from, $limit";
...
//  $sql2 = "SELECT COUNT(email) FROM (SELECT * FROM candidateregdata WHERE (`qua` LIKE  '%$searchQua%') ORDER BY email";
    $sql2 = "SELECT COUNT(email) FROM candidateregdata WHERE ( `qua` LIKE  '%$searchQua%' AND `totalExp` LIKE  '%$searchExp%')  ORDER BY email ";
